...
const ContainerClient = require('./container-client')
...
// split blocks larger than this size if savings result
const splitBlockSize = 64*1024
...
module.exports = class Container {
     ...
     * @param {string} args.newReplaceToken
     * @param {string} args.replaceLink
     * @param {string} args.replaceToken
    ...
    constructor (args = {}) {
        ...
        this.publicId = ''
        // if replace link is set then create client instance for container
        // that should be replaced
        if (args.replaceLink) {
            // create client for container
            this.replaceContainer = new ContainerClient(args.replaceLink, {
                client: args.client
            })
        }
        else {
        ...
        // link for container to replace
        this.replaceLink = args.replaceLink || ''
        // token to authorize replace
        this.replaceToken = args.replaceToken || ''
    ...
    async finalize () {
        ...
        // if container is replacement create replace entry
        if (this.replaceContainer) {
            // link for replacement does not include key because replacement
            // must use same key as original
            const link = [
                this.head.size,
                this.contentType,
                this.head.blocks[0],
            ...
            await this.client.createReplace({
                link: link,
                originalId: this.meta.originalId,
                parentId: this.meta.parentId,
                token: this.replaceToken,
            })
        }
        // create replace token
        const replaceToken = await this.client.createReplaceToken(this.privateId, this.replaceToken)
        // save replace token
        this.replaceToken = replaceToken.token
    }
    ...
    async initKey () {
        assert(!this.keyBuffer, 'keyBuffer already initialized')
        // if this is replacement must use same password as original
        if (this.replaceContainer) {
            this.key = this.replaceContainer.link.password
        }
        else {
        ...
        // created derived key
        this.keyBuffer = await Crypto.deriveKey(Buffer.from(this.key), this.saltBuffer)
    }
    ...
    async prepareMeta () {
        ...
        // if this is replacement set parent and original id
        if (this.replaceContainer) {
            this.meta.originalId = this.replaceContainer.meta.originalId || this.replaceContainer.privateId
            this.meta.parentId = this.replaceContainer.privateId
        }
    ...
    async publishData (bytes) {
        ...
        let bufferedDataLength = this.getBuffersLength()
        ...
        // otherwise get best fit for data length
            ...
            while (bufferedDataLength > 0) {
                // get block size that will fit data
                const blockSize = Block.getBlockSize(bufferedDataLength)
                // split data into multiple blocks for better efficiency
                if (Block.shouldSplitBlock(blockSize)) {
                    // get number of bytes for next smaller block size
                    const blockBytes = Block.getBlockSizes()[blockSize.size - 1]
                    // get sequential number for this block
                    const blockNum = this.dataBlocks.length
                    // if block is being split some files may no longer be in block
                    for (const file of this.files) {
                        if (file.block === blockNum && file.offset >= blockBytes) {
                            // move file to next block
                            file.block += 1
                            // remove bytes of current block from offset
                            file.offset -= blockBytes
                        }
                    }
                    // publish partial data
                    const dataBuffer = this.getDataBuffer(blockBytes)
                    assert(dataBuffer.length === blockBytes, 'data buffer length mismatch')
                    publishedDataBlocks.push( await this.publishDataBlock(dataBuffer) )
                }
                // publish all data
                else {
                    const dataBuffer = this.getDataBuffer(bufferedDataLength)
                    assert(dataBuffer.length === bufferedDataLength, 'data buffer length mismatch')
                    publishedDataBlocks.push( await this.publishDataBlock(dataBuffer) )
                }
                // get remaining data
                bufferedDataLength = this.getBuffersLength()
            }
        }
    ...
    setContentType (contentType) {
        assert(!this.done, 'cannot contentType when done')
