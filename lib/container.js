...
module.exports = class Container {
    ...
    constructor (args = {}) {
        ...
        this.metaLength = 0
        // token used to find replacement block
        this.replaceToken = ''
        // secret used to sign replacement token submit
        this.replaceSecret = ''
    ...
    getHeadLength () {
        ...
        if (this.padBuffer) {
            headLength += this.padBuffer.length
        }
        // meta length (uint32)
        headLength += 4
    ...
    async getReplace () {
        console.log(`Getting replace token from ${this.api}`)
        // get replace token/secret
        const res = await requestPromise({
            json: true,
            uri: `${this.api}/replace/token`,
        })

        this.replaceToken = res.token
        this.replaceSecret = res.secret
    }
    ...
    async initPad () {
        ...
        if (this.padBuffer) return
        // get random bytes of specified length
        this.padBuffer = await crypto.randomBytes(randomInt(2, 4096))
    }
    ...
    async publishHead () {
        ...
        let offset = 0
        // replace token
        const replaceTokenBuffer = Buffer.from(this.replaceToken, 'hex')
        replaceTokenBuffer.copy(plainBuffer, offset)
        offset += replaceTokenBuffer.length
        // chat key
        this.chatKeyBuffer.copy(plainBuffer, offset)
        offset += this.chatKeyBuffer.length
        // pad length
        plainBuffer.writeUInt16BE(this.padBuffer.length, offset)
        offset += 2
        // pad
        this.padBuffer.copy(plainBuffer, offset)
        offset += this.padBuffer.length
        // meta length
