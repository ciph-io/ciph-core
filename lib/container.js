...
/* app modules */
const CiphContainerClient = require('./client/node/ciph-container-client')
...
module.exports = class Container {
    ...
    async initKey () {
        ...
        assert(this.key.match(/^[a-fA-F0-9]+$/), 'key must be hex string')
        assert(this.key.length % 2 === 0, 'key length must be multiple of 2')
        // create salt
    ...
    async publishHead () {
        ...
        console.log(plainBuffer.slice(0, offset).toString('hex'))
        // get hash of head data
        const headSHA = sha256([metaBuffer, plainBuffer.slice(0, offset)])
        console.log("YYY", headSHA.toString('hex'))
