...
module.exports = class Client {
    ...
    constructor (args = {}) {
        ...
        Client.setProxyHost(true, this.api)
    }
    ...
    async getBlock (size, blockId, retry) {
        try {
            console.log(`get ${proxyHost}/get-proxy/${size}/${blockId}.ciph`)
            // download block
            const block = await request({
                encoding: null,
                headers: this.user.getAuthHeaders(),
                timeout: 10000,
                uri: `${proxyHost}/get-proxy/${size}/${blockId}.ciph`,
            })
            ...
            if (retry) {
                ...
                // try different proxy on error
                Client.setProxyHost(true)
                // retry
     ...
     *
     */
    static setProxyHost (force, api) {
        if (proxyHost && !force) {
            return
        }
        let dev = false
        // if in dev use dev proxy host
        if (api === 'https://dev.ciph.io') {
